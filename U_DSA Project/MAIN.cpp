...
struct Appointment {
    ...
    Appointment(int appointmentId, int patientId, int doctorId, string date, string time)
        : appointmentId(appointmentId), patientId(patientId), doctorId(doctorId), date(date), time(time), next(nullptr) {}
};
...
void saveData() {
    ...
    Patient* pTemp = patientHead;
    while (pTemp) {
        patientFile << pTemp->id << " " << pTemp->name << " " << pTemp->age << " " << pTemp->ailment << endl;
        pTemp = pTemp->next;
    }
    patientFile.close();
